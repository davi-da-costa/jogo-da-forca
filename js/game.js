...
function adivinha(letters) {
  ...
  input.addEventListener("input", (e) => {
    ...
    let letra_digitada = value[value.length - 1];

    if (
      (letra_digitada.charCodeAt(0) >= 65 &&
        letra_digitada.charCodeAt(0) <= 90) ||
      (letra_digitada.charCodeAt(0) >= 97 &&
        letra_digitada.charCodeAt(0) <= 122)
    ) {
      let letra_repetida = false;
      if (
        letras_digitadas.find((letra) => letra === letra_digitada.toUpperCase())
      ) {
        letra_repetida = true;
      }
      ...
      if (!letra_repetida) {
        ...
        p.textContent = letra_digitada.toUpperCase();
        document.querySelector(".past_letters").appendChild(p);
        letras_digitadas.push(letra_digitada.toUpperCase());
      }
      ...
      for (let i = 0; i < letters.length; i++) {
        let letter_div = document.getElementById(`l${i + 1}`);
        if (letra_digitada === letters[i].toLowerCase()) {
